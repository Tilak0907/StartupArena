...
const trlDoc = await getDoc(
doc(db, "trlLevels", profileData.userId)
);
...
const matrixDoc = await getDoc(
doc(db, "evaluationMatrix", profileData.userId)
);
...
<div
key={key}
className={`matrix-item ${value ? "matrix-true" : "matrix-false"}`}
>
...
<>
<button onClick={handleSaveMatrix} className="primary-btn">
Save Changes
</button>
...
<p>
<b>Date:</b>{" "}
{new Date(reviewDate.seconds * 1000).toLocaleString()}
</p>
)}
</>
)}
